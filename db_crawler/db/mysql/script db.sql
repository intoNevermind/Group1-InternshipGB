CREATE SCHEMA IF NOT EXISTS `persons_statistics`;

CREATE TABLE IF NOT EXISTS `persons_statistics`.`persons` (
...
CREATE TABLE IF NOT EXISTS `persons_statistics`.`keywords` (
  `ID` INT NOT NULL AUTO_INCREMENT,
  `Name` NVARCHAR(2048) NOT NULL,
...
CREATE TABLE IF NOT EXISTS  `persons_statistics`.`sites` (
  `ID` INT NOT NULL AUTO_INCREMENT,
  `Name` NVARCHAR(2048) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `persons_statistics`.`pages` (
  `ID` INT NOT NULL AUTO_INCREMENT,
  `URL` NVARCHAR(2048) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `persons_statistics`.`personpagerank` (
  `PersonID` INT NOT NULL COMMENT 'Идентификатор личности, которой соответствует данное ключевое слово. Является внешним ключом к таблице Persons.',
