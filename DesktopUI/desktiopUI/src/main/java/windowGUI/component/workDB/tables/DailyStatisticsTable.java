...
import windowGUI.component.workDB.restApi.PojoDailyStatistics;
import windowGUI.component.workDB.restApi.RestApiForDailyStatistics;
        ...
        try {
            ...
            for (int j = 0; j < list.size(); j++) {
                ...
                if(namePerson.equals(LIST_NAME.get(j)) &&  LIST_DAILY_RANK.get(j) != 0){
                    LIST_DATE_AND_DAILY_RANK.put(LIST_NAME.get(j), LIST_DAILY_RANK.get(j));
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
